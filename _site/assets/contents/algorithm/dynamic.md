...
동적계획법(DP)은 큰 문제를 작은 부분 문제로 나누어 해결하고, 그 결과를 저장하여 재사용하는 최적화 기법이다.   
중복 계산을 제거하여 지수 시간을 다항 시간으로 줄여준다.
...
`#Java` `#DP` `#DynamicProgramming` `#동적계획법` `#메모이제이션` `#Memoization` `#점화식`  
 `#배낭문제` `#Knapsack` `#LCS` `#최장공통부분수열` `#알고리즘` `#코딩테스트`